...
public class DeliveryController {
    ...
    public CompletableFuture cancelDelivery(@PathVariable("id") String id)
        ...
        cancelDeliveryCommand.setOrderId(id);
        // send command
    ...
    EntityModel<DeliveryAggregate> hateoas(DeliveryAggregate resource) {
        ...
        model.add(
            Link.of("/deliveries/" + resource.getOrderId()).withSelfRel()
        );

        model.add(
            Link
                .of("/deliveries/" + resource.getOrderId() + "/canceldelivery")
                .withRel("canceldelivery")
        );

        model.add(
            Link
                .of("/deliveries/" + resource.getOrderId() + "/events")
                .withRel("events")
        );
